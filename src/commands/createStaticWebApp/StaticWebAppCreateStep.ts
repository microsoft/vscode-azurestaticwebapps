...
export class StaticWebAppCreateStep extends AzureWizardExecuteStep<IStaticWebAppWizardContext> {
  ...
  public async execute(
    ...
    if (context.shouldInitLogicApp) {
      ...
      const { data } = await octokitClient.repos.get({
        owner: branch_owner,
        repo,
      });
    ...
    const creatingSwa: string = localize(
      "creatingSwa",
    ...
    if (context.shouldInitLogicApp) {
      const staticSiteLinkedBackendEnvelope = {
        backendResourceId: context.shouldInitLogicApp.backendResourceId,
        region: context.shouldInitLogicApp.region,
      };
      ...
      try {
        const result = await client.staticSites.beginLinkBackendAndWait(
          nonNullValueAndProp(context.resourceGroup, "name"),
          nonNullValueAndProp(context.staticWebApp, "name"),
          context.shouldInitLogicApp.name,
