...
export class GitignoreCreateStep extends AzureWizardExecuteStep<IStaticWebAppWizardContext> {
    public priority: number = 300;

    public async execute(wizardContext: IStaticWebAppWizardContext): Promise<void> {
        const fsPath: string = nonNullProp(wizardContext, 'fsPath');
        const gitignorePath: string = join(fsPath, '.gitignore');
        await writeFile(gitignorePath, defaultGitignoreContents);
    }
...
const defaultGitignoreContents: string =
`# Logs
logs
*.log
npm-debug.log*
