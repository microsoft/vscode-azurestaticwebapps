...
export async function rerunAction(context: IActionContext, node?: ActionTreeItem): Promise<void> {
    ...
    if (!node) {
        node = await ext.rgApi.tree.showTreeItemPicker<ActionTreeItem>(ActionTreeItem.contextValueCompleted, { ...context, suppressCreatePick: true, noItemFoundErrorMessage });
    }
...
export async function cancelAction(context: IActionContext, node?: ActionTreeItem): Promise<void> {
    ...
    if (!node) {
        node = await ext.rgApi.tree.showTreeItemPicker<ActionTreeItem>(ActionTreeItem.contextValueInProgress, { ...context, suppressCreatePick: true, noItemFoundErrorMessage });
    }
