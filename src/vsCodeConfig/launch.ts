...
const compoundsKey: string = 'compounds';
...
export function getCompoundConfigs(folder: WorkspaceFolder): CompoundConfiguration[] {
    return getLaunchConfig(folder).get(compoundsKey) ?? [];
}

export async function updateCompoundConfigs(folder: WorkspaceFolder, configs: CompoundConfiguration[]): Promise<void> {
    await getLaunchConfig(folder).update(compoundsKey, configs);
}

export function isCompoundConfigEqual(c1: CompoundConfiguration, c2: CompoundConfiguration): boolean {
